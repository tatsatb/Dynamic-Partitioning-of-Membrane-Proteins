...
param.D_BS1 = 0.45;        % diffusion constant of B:PP
param.D_FS1 = 0.45;        % diffusion constant of F:PP

% S2: LP_u, BS2: LP_b, 
param.D_S2  = 0.45;  % diffusion constant of unbound LP
param.D_BS2 = 0.45;  % diffusion constant of B:LP
param.D_FS2 = 0.45;  % diffusion constant of F:LP
...
figure;
hold on
plot(comb_data.tspan(1:10:end),(comb_data.sum_F_molecule(1:10:end)/max(comb_data.sum_F_molecule)))
plot(comb_data.tspan(1:10:end),comb_data.sum_B_molecule(1:10:end)/max(comb_data.sum_B_molecule))
plot(comb_data.tspan(1:10:end),comb_data.sum_R_molecule(1:10:end)/max(comb_data.sum_R_molecule))
ylim([0 1.1])
set(gca,'TickDir','out');
%print -painters -dsvg Fig_supp13_B1.svg
...
figure;
hold on
plot(comb_data.tspan(1:10:end),(S1_membrane(1:10:end)))
plot(comb_data.tspan(1:10:end),(S2_membrane(1:10:end)))
ylim([0.5 1.1])
set(gca,'TickDir','out');
%print -painters -dsvg Fig_supp13_B2.svg
...
%%
    ...
    figure('color','white')    
    hold on
    plot(comb_data.tspan,F_smooth(:,:)/max(F_smooth(:)),'g-','linewidth',1)
    plot(comb_data.tspan,B_smooth(:,:)/max(B_smooth(:)),'r-','linewidth',1)
    plot(comb_data.tspan,R_smooth(:,:)/max(R_smooth(:)),'b-','linewidth',1)
    ylim([0 1])
    set(gca,'TickDir','out');
    %print -painters -dsvg Fig_supp13_C1.svg

%%
    figure('color','white')    
    hold on

    plot(comb_data.tspan,(BS1_smooth(:,:)+FS1_smooth(:,:)+S1_smooth(:,:))/max(S1_total(:)),'r-','linewidth',1)
    plot(comb_data.tspan,(BS2_smooth(:,:)+FS2_smooth(:,:)+S2_smooth(:,:))/max(S2_total(:)),'g-','linewidth',1)
    ylim([0 1])
    set(gca,'TickDir','out');
    %print -painters -dsvg Fig_supp13_C2.svg
...
% Ts1 = 10; % time scaling for PP dyanamics
...
% r12  = 'S1 > 2*(e3/vol)*B*S1 > BS1';
% r13  = 'BS1 > 1.7*(e4)*BS1 > S1';
% r14  = 'S1 > (e5/vol)*F*S1 > FS1';
% r15  = 'FS1 > 2.6*(e6)*FS1 > S1';
%--------------------------------------------------------------------------
d1 = param.d1;
d2 = param.d2;
d3 = param.d3/k;
...
function data_visualization(comb_data,field_names,param)
    ...
    Data_folder = fullfile('Fig_Supp_13',time_stamp_for_file);
